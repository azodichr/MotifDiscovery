"""
PURPOSE:
Find k-mers enriched in your positive dataset and build feature dataframe out of enriched k-mers
...
INPUT:
  ...
  -k        List of kmers to start with (/mnt/home/azodichr/01_CombinedStress/6mers.txt)
...
OUTPUT:
  ...
  -SAVE_FETresults.txt    Results for features enriched with fisher's exact test: Feature, TP, FN, TN, FP, percent_postives, pvalue


                      |  Contains Kmer    |  Lacking Kmer
-------------------------------------------------------------
...
def Find_Enrich(POS, NEG, km, PVAL, SAVE):
  ...
  outFISH.write('feature\tPosPresent\tPosAbsent\tNegAbsent\tNegPresent\tpercent_postives\tpvalue')
  count = 0
  ...
  for k in positive_present:
    try:
      ...
      outFISH.write('\n%s\t%d\t%d\t%d\t%d\t%.7f\t%.7f' % (k, TP,FN, TN, FP, percent, pvalue))
...
def Make_DF(K, PVAL, SAVE, POS, NEG):
